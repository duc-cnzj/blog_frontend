...
export default {
  methods: {
    loading () {
      ...
      setTimeout(this.fill, (truckLoopDur * 1e3) / 4)
    },
    fill () {
      let truckLoopDur = 10
      ...
      if (step < percents.length) {
        setTimeout(this.fill, (truckLoopDur * 1e3) / 2)
      }
    }
  }
}
...
.dump-truck {
  ...
  &:last-of-type {
    animation: backupR $truckLoopDur $truckLoopDur/2 ease-in-out $truckRounds
      forwards;
    left: 50%;
...
.front {
  background: linear-gradient($r, $r) 2.7em 0.75em / 0.2em 2em,
...
.wheel {
  background: radial-gradient(closest-side, $bk 9%, $bkT 10%),
    radial-gradient(closest-side, $lgy 29%, $lgyT 30%),
    linear-gradient(90deg, $w 49%, $wT 50%),
    radial-gradient(closest-side, $bk 49%, $bkT 50%);
  border-radius: 50%;
  bottom: 0;
  width: 2.4em;
  height: 2.4em;
